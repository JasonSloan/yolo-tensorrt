...
// 重载update函数, 调试用
vector<STrack> BYTETracker::update(const vector<Object>& objects, int& count, int& agg_iters){
	...
	if (count == agg_iters){
		spdlog::info("activated_stracks size: {}, refind_stracks size: {}, removed_stracks size: {}, lost_stracks size: {}", activated_stracks.size(), refind_stracks.size(), removed_stracks.size(), lost_stracks.size());
		spdlog::info("detections size: {}, detections_low size: {}, detections_cp size: {}, tracked_stracks_swap size: {}", detections.size(), detections_low.size(), detections_cp.size(), tracked_stracks_swap.size());
		spdlog::info("resa size: {}, resb size: {}, output_stracks size: {}, unconfirmed size: {}", resa.size(), resb.size(), output_stracks.size(), unconfirmed.size());
		spdlog::info("tracked_stracks size: {}, strack_pool size: {}, r_tracked_stracks size: {}", tracked_stracks.size(), strack_pool.size(), r_tracked_stracks.size());
		spdlog::info("this->tracked_stracks size: {}, this->lost_stracks size: {}, this->removed_stracks size: {}", this->tracked_stracks.size(), this->lost_stracks.size(), this->removed_stracks.size());
		count = 0;
	}
